...
#   resume=True \
#   weight=exp_runs/lang_pretrainer/multi_teacher/concat_dataset/dino-enc-pretrain-scan-ppv2-mcmc-pts-params-submit/model/model_last.pth \
# Optional wandb settings (wandb is enabled by default):
#   enable_wandb=False \
#   wandb_id=<wandb_run_id>

srun python -u -m tools.train \
  --config-file configs/chorus/concat_dataset/dino-only-enc-pretrain-scan-ppv2-mcmc-pts-params.py \
  --options \
    save_path=exp_runs/lang_pretrainer/chorus/concat_dataset/dino-only-enc-pretrain-scan-ppv2-mcmc-pts-params \
