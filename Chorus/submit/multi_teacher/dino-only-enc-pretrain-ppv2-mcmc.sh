...
#   resume=True \
#   weight=exp_runs/lang_pretrainer/multi_teacher/dino-only-enc-pretrain-ppv2-mcmc/model/model_last.pth \
# Optional wandb settings (wandb is enabled by default):
#   enable_wandb=False \
#   wandb_id=<wandb_run_id>

python -u -m tools.train \
  --config-file configs/chorus/dino-only-enc-pretrain-ppv2-mcmc.py \
  --options \
    save_path=exp_runs/lang_pretrainer/chorus/dino-only-enc-pretrain-ppv2-mcmc \
