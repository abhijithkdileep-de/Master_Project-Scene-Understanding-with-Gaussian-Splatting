...
#   resume=True \
#   weight=exp_runs/lang_pretrainer/multi_teacher/lang-only-enc-pretrain-sn-mcmc/model/best_lang.pth \
# Optional wandb settings (wandb is enabled by default):
#   enable_wandb=False \
#   wandb_id=<wandb_run_id>

python -u -m tools.train \
  --config-file configs/chorus/lang-only-enc-pretrain-scannet-mcmc.py \
  --options \
    save_path=exp_runs/lang_pretrainer/chorus/lang-only-enc-pretrain-scannet-mcmc \
