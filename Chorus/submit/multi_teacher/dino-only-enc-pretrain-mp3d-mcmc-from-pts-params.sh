...
#   resume=True \
#   weight=exp_runs/lang_pretrainer/multi_teacher/dino-enc-pretrain-matt-mcmc-from-pts-params-submit-new/model/epoch_80.pth \
# Optional wandb settings (wandb is enabled by default):
#   enable_wandb=False \
#   wandb_id=<wandb_run_id>

python -u -m tools.train \
  --config-file configs/chorus/dino-only-enc-pretrain-mp3d-mcmc-from-pts-params.py \
  --options \
    save_path=exp_runs/lang_pretrainer/chorus/dino-only-enc-pretrain-mp3d-mcmc-from-pts-params \
