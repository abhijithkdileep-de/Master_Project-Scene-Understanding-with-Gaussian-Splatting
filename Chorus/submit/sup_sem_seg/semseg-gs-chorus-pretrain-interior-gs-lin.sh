...
batch_size_val=$((4*gpu_num))
batch_size_test=$((1*gpu_num))
num_worker=$((8*gpu_num))

export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
python -u -m tools.train \
  --config-file configs/interior_gs/semseg-gs-ptv3m2-interior-gs-lin.py \
  --options \
    save_path=exp_runs/sup_sem_seg/interior-gs-lin-probe-for-joint-lang-enc-pretrain \
