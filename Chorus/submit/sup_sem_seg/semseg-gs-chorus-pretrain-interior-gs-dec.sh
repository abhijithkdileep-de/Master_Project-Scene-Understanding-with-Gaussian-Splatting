...
batch_size_val=$((3*gpu_num))
batch_size_test=$((1*gpu_num))
num_worker=$((8*gpu_num))

export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
python -u -m tools.train \
  --config-file configs/interior_gs/semseg-gs-ptv3m2-interior-gs-dec.py \
  --options \
    save_path=exp_runs/sup_sem_seg/interior-gs-dec-probe-for-joint-lang-enc-pretrain \
    ...
    weight=exp_runs/lang_pretrainer/multi_teacher/concat_dataset/lang-enc-pretrain-multi-teacher-scan-ppv2-matt/model/model_last.pth \
  --num-gpus $gpu_num \
