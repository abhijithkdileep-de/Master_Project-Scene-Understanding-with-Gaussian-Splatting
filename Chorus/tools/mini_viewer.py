...
from scripts.gaussian_io import get_default_output_dir, infer_scene_name
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Launch installed Mini Viewer on a source 3DGS scene and Chorus "
            "inference features."
        ),
    )
    parser.add_argument(
        "--input-root",
        default=None,
        help="Original scene directory with .npy files, or the raw/compressed .ply used for inference.",
    )
    parser.add_argument(
        "--precompile-gsplat",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<output-dir>/<scene>_lang_feat.pt, matching Chorus inference."
        ),
    )
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        ...
        help="Mini Viewer feature/query family. Chorus lang features use siglip2.",
    )
    ...
    parser.add_argument("--siglip-model", default=None, help="Optional SigLIP/SigLIP2 model id.")
...
def _require_installed_miniviewer() -> None:
    ...
    raise ModuleNotFoundError(
        ...
        "Install it with: python -m pip install -e /path/to/Mini_Viewer"
    )
...
def build_command(args: argparse.Namespace, extra_args: list[str]) -> tuple[list[str], Path | None]:
    ...
    if feature_path is not None and not feature_path.exists() and not args.dry_run:
        raise FileNotFoundError(
            ...
            f"{feature_path}. Run Chorus inference first or pass --feature-path."
        )
