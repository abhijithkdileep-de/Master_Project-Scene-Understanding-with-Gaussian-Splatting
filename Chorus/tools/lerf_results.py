"""SceneSplat-style comparison artifacts for Chorus LERF evaluation.
...
def save_panel(rgb_path, gt, prediction, target, title):
    ...
    coverage = 100.0 * np.count_nonzero(prediction) / prediction.size
    labels = ['RGB Image', 'Ground Truth', f'Prediction ({coverage:.1f}% pixels)',
              'Overlay: GT green, prediction red']
...
def save_histogram(scores, threshold, path):
    counts, edges = np.histogram(scores, bins=100, range=(0,1))
    ...
    x = 50+int(900*threshold)
    draw.line((x,40,x,400),fill='red',width=3)
    draw.text((50,12),f'THGS relevance histogram; selected threshold={threshold:.6f}',fill='black')
    draw.text((50,420),'0                 Relevance score (linear count axis)                 1',fill='black')
    canvas.save(path)
    return counts, edges


def save_query_artifacts(output_dir, query, all_queries, scores, summary, results, scene, args):
    ...
    def artifact(key, filename):
        ...
        return path
    selection = (scores >= summary['threshold']).astype(np.uint8)
    indices = np.flatnonzero(selection)
    indices = indices[np.argsort(-scores[indices], kind='stable')]
    np.save(artifact('similarity',f'{slug}_similarity.npy'),scores)
    ...
    write_json(artifact('retrieval',f'{slug}_retrieval.json'), dict(
        method='Chorus', scene=args.scene, query=query, selection_method='THGS_query_similarity_threshold',
        neighbor_voting=False, vote_k=None, threshold=summary['threshold'],
        threshold_source=summary['threshold_source'], num_selected=len(indices),
        selected_fraction=len(indices)/len(scores), gaussian_indices=indices.tolist(),
        similarity_scores=scores[indices].tolist(), index_space='evaluation_geometry_rows'))
    counts, edges = save_histogram(scores,summary['threshold'],artifact('histogram',f'{slug}_similarity_histogram.png'))
    np.savez(artifact('histogram_data',f'{slug}_similarity_histogram.npz'),counts=counts,edges=edges)
    # A binary XYZ/RGB point cloud for inspecting continuous similarity.
    import cv2
    ...
    with artifact('similarity_ply',f'{slug}_similarity.ply').open('wb') as stream:
...
def save_comparison_reports(output_dir, report):
    ...
    metrics=dict(method='Chorus',scene=report['scene'],protocol=report['protocol'],threshold=report['threshold'],neighbor_voting=False,
    ...
    fields=['Method','Scene','Query','Mean_IoU','mAcc','Threshold','Threshold_Source',
            'Raw_Selected_Gaussians','Voted_Selected_Gaussians','Neighbor_Voting']
    with (output_dir/'per_query_results.csv').open('w',newline='') as stream:
        ...
        for row in rows:
            writer.writerow(['Chorus',report['scene'],row['query'],round(row['mean_iou'],6),round(row['mAcc'],6),
                row['threshold'],row['threshold_source'],row['selected_gaussians'],'',False])
    manifest=dict(method='Chorus',scene=report['scene'],protocol=report['protocol'],neighbor_voting=False,
        ...
        notes={'precision@IoU':'Fraction of query-mean IoUs meeting the cutoff, matching SceneSplat naming.',
