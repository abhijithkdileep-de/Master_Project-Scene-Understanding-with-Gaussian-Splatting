...
class SGIFormerLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # Track all loss components for NaN checking
        all_loss_components = []
        loss_names = []
        
        # Add main loss components
        all_loss_components.extend([
            loss_dict["loss_cls"],
            loss_dict["loss_mask"],
            loss_dict["loss_dice"],
            loss_dict["loss_score"]
        ])
        loss_names.extend(["loss_cls", "loss_mask", "loss_dice", "loss_score"])
        ...
        if "aux_pred_list" in pred:
            ...
            for i, pred_inst_info_ in enumerate(pred["aux_pred_list"]):
                aux_loss, aux_loss_dict, _ = self.get_loss(
                    pred_inst_info_, gt_inst_info, matched_idx_list
                )
                # Check auxiliary loss for NaN
                if torch.isnan(aux_loss).any() or torch.isinf(aux_loss).any():
                    print(f"Warning: NaN/Inf in auxiliary loss {i}: {aux_loss.item()}")
                    print(f"  Aux loss components: cls={aux_loss_dict['loss_cls'].item()}, "
                        f"mask={aux_loss_dict['loss_mask'].item()}, "
                        f"dice={aux_loss_dict['loss_dice'].item()}, "
                        f"score={aux_loss_dict['loss_score'].item()}")
                    aux_loss = torch.tensor(0.0, device=loss.device)
                aux_loss_total = aux_loss_total + aux_loss
            all_loss_components.append(aux_loss_total)
            loss_names.append("aux_loss_total")
        ...
        if "seg_logits" in pred:
            if pred["seg_logits"] is not None:
                if gt_point_info["segment"].max() >= 0:
                    ...
                    if torch.isnan(pred["seg_logits"]).any():
                        print("Warning: NaN in seg_logits before loss computation")
                        loss_seg = torch.tensor(0.0, device=loss.device)
                    else:
                        ...
                        if torch.isnan(loss_seg).any() or torch.isinf(loss_seg).any():
                            print(f"Warning: NaN/Inf in segmentation loss: {loss_seg.item()}")
                            loss_seg = torch.tensor(0.0, device=loss.device)
                else:
                    loss_seg = torch.tensor(0.0, device=loss.device)
            else:
            ...
            all_loss_components.append(self.loss_weight[4] * loss_seg)
            loss_names.append("weighted_loss_seg")
            loss_dict["loss_seg"] = loss_seg

        # Handle bias loss
        if "bias" in pred:
            if pred["bias"] is not None:
                # Check for NaN in bias predictions
                if torch.isnan(pred["bias"]).any():
                    print("Warning: NaN in bias predictions")
                    ...
                    # Check for NaN after computation
                    if torch.isnan(loss_bias).any() or torch.isinf(loss_bias).any():
                        print(f"Warning: NaN/Inf in bias loss: {loss_bias.item()}")
                        loss_bias = torch.tensor(0.0, device=loss.device)
            else:
                loss_bias = torch.tensor(0.0, device=loss.device)
            
            all_loss_components.append(self.loss_weight[5] * loss_bias)
            loss_names.append("weighted_loss_bias")
            loss_dict["loss_bias"] = loss_bias

        # Rebuild total loss with NaN checking
        total_loss = torch.tensor(0.0, device=loss.device, requires_grad=True)
        
        # Check and accumulate each component
        for i, (component, name) in enumerate(zip(all_loss_components, loss_names)):
            if torch.isnan(component).any() or torch.isinf(component).any():
                print(f"NaN/Inf detected in {name}: value={component.item() if component.numel() == 1 else 'tensor'}")
                # Replace with zero but keep gradient flow
                component = torch.tensor(0.0, device=loss.device, requires_grad=True)
            else:
                # Clamp extreme values to prevent future overflow
                component = torch.clamp(component, min=-100, max=100)
            
            total_loss = total_loss + component
        
        # Final check on total loss
        if torch.isnan(total_loss).any() or torch.isinf(total_loss).any():
            print(f"Critical: Total loss is NaN/Inf even after component checking!")
            print(f"Loss dict values: {[(k, v.item() if v.numel() == 1 else 'tensor') for k, v in loss_dict.items()]}")
            # Fall back to a small loss value to keep training going
            total_loss = torch.tensor(0.01, device=loss.device, requires_grad=True)
        
        loss_dict["loss"] = total_loss
        
        # Print loss components periodically for debugging (optional)
        # Uncomment the following for detailed debugging:
        # if torch.rand(1).item() < 0.01:  # Print 1% of the time
        #     print(f"Loss components: cls={loss_dict['loss_cls'].item():.4f}, "
        #           f"mask={loss_dict['loss_mask'].item():.4f}, "
        #           f"dice={loss_dict['loss_dice'].item():.4f}, "
        #           f"score={loss_dict['loss_score'].item():.4f}, "
        #           f"seg={loss_dict.get('loss_seg', 0):.4f}, "
        #           f"bias={loss_dict.get('loss_bias', 0):.4f}, "
...
class HungarianMatcher:
    ...
    def __call__(self, pred_inst, gt_inst, **kwargs):
        ...
        # Check for empty masks BEFORE computing costs
        if gt_inst["mask"].numel() == 0 or pred_inst["mask"].numel() == 0:
            return inst_cls.new_empty((0,), dtype=torch.int64), inst_cls.new_empty(
                (0,), dtype=torch.int64
            )
        
        # Check for NaN in inputs before computing costs
        if torch.isnan(pred_inst["cls"]).any() or torch.isnan(pred_inst["mask"]).any():
            print("NaN detected in predictions before matching!")
            # Initialize with default matching or skip
            n = min(len(pred_inst["cls"]), inst_num)
            return torch.arange(n, dtype=torch.int64), torch.arange(n, dtype=torch.int64)
