...
class PositionEmbeddingCoordsSine(nn.Module):
    def __init__(
        ...
        if pos_type == "fourier":
            ...
            B *= gauss_scale
            B = B / (B.norm(dim=0, keepdim=True) + 1e-8) * gauss_scale # # Add normalization to prevent large values
    ...
    def get_sine_embeddings(self, xyz, num_channels, input_range):
        ...
        for d in range(xyz.shape[2]):
            ...
            if cdim != prev_dim:
                ...
                dim_t = torch.exp(torch.log(torch.tensor(self.temperature)) * (2 * (dim_t // 2) / cdim)) # Use log-space computation
    ...
    def get_fourier_embeddings(self, xyz, num_channels=None, input_range=None):
        ...
        xyz *= 2 * torch.pi
        xyz = torch.clamp(xyz * 2 * torch.pi, min=-1000, max=1000) 
...
class SGIFormer(nn.Module):
    ...
    def forward(self, data_dict, return_point=False):
        ...
        if not self.training:
            ...
            # )
            mask_sum = (pred_mask > 0).sum(1)
            mask_scores = torch.where(
                mask_sum > 0,
                (pred_mask_sigmoid * (pred_mask > 0)).sum(1) / mask_sum,
