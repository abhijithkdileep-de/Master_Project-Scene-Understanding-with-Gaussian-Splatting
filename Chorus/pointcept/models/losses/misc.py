...
class SmoothL1Loss(nn.Module):
    ...
    Args:
        ...
        beta (float, optional): The threshold at which to change between
            L1 and L2 loss. The value must be non-negative. Default: 1.0.
        loss_weight (float, optional): The weight of the loss. Default: 1.0.
    """
    def __init__(
        self,
        size_average=None,
    ...
    def forward(self, pred, target, **kwargs):
        ...
        """
        # check if valid_feat_mask is in kwargs
        if self.masking: 
            if "valid_feat_mask" in kwargs:
                valid_feat_mask = kwargs["valid_feat_mask"]
                pred = pred[valid_feat_mask]
                target = target[valid_feat_mask]
                if len(target) == 0:
...
class AggregatedContrastiveLoss(nn.Module):
    def __init__(
        ...
        if "last_" in self.schedule:
            ...
            _emit_loss_log(
                "Contrastive loss apply in last %s%% of training.",
                self.schedule.split("_")[-1],
            )
        elif self.schedule == "all":
            _emit_loss_log("Contrastive loss is applied in all epochs.")
        elif self.schedule == "skip":
            _emit_loss_log("Contrastive loss is skipped.")
