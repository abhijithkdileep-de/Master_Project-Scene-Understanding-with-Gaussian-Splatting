...
class DefaultDataset(Dataset):
    ...
    def get_data_list(self, filtered_scene=None):
        ...
        data_list = sorted(data_list) # ensure the order is the same
        return data_list
    ...
    def prepare_test_data(self, idx):
        ...
        # limit the test data throughput
        input_num = data_dict["coord"].shape[0]
        if "limit_num" in self.test_cfg and input_num > self.test_cfg.limit_num:
            # enlarge the grid_size in GridSample and voxelize
            self.test_voxelize.grid_size *= 2
            for transform in self.transform.cfg:
                if transform.get('type') == 'GridSample':
                    transform['grid_size'] = transform['grid_size'] * 2
            self.logger.info(
                f"Test data {data_dict['name']} has {input_num} input, exceeds the limit {self.test_cfg.limit_num}. "
                f"Enlarging grid size by 2."
            )
        data_dict = self.transform(data_dict)
        result_dict = dict(
        ...
        if "origin_dino_feat" in data_dict:
            result_dict["origin_dino_feat"] = data_dict.pop("origin_dino_feat")
        ...
        for data in data_dict_list:
            if self.test_voxelize is not None:
                data_part_list = self.test_voxelize(data) # 'test' mode GridSample here
            else:
...
class ConcatDataset(Dataset):
    ...
    def get_data(self, idx):
        ...
        dataset_idx = int(dataset_idx)  # ensure it's an integer
        data_idx = int(data_idx)  # ensure it's an integer
