...
if __name__ == "__main__":
    ...
    with mp.Pool(processes=config.num_workers) as pool:
        ...
        for i in range(len(data_list)):
            result = pool.apply_async(
                ...
                args=(
                    data_list[i],
                    split_list[i],
                    str(config.dataset_root),  # Convert Path to str
                    str(config.output_root),   # Convert Path to str
                    label_mapping,
                    class2idx,
                    config.ignore_index,
                )
        ...
        for result in tqdm(results, desc="Processing scenes"):
            result.get(timeout=600)  # 10 minute timeout
    # for i in tqdm(range(len(data_list)), desc="Processing scenes"):
    #     parse_scene(
    #         data_list[i],
    #         split_list[i],
    #         config.dataset_root,
