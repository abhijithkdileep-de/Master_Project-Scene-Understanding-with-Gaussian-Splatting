...
def default_argument_parser(epilog=None, prog=None):
    example_prog = prog or sys.argv[0]
    ...
    Run on single machine:
        $ {example_prog} --num-gpus 8 --config-file cfg.yaml
    Change some config options:
        $ {example_prog} --config-file cfg.yaml MODEL.WEIGHTS /path/to/weight.pth SOLVER.BASE_LR 0.001
    Run on multiple machines:
        (machine0)$ {example_prog} --machine-rank 0 --num-machines 2 --dist-url <URL> [--other-flags]
        (machine1)$ {example_prog} --machine-rank 1 --num-machines 2 --dist-url <URL> [--other-flags]
    """,
    ...
    parser.add_argument(
        "--multi_node", action="store_true", help="do not use distributed training"
    )
    return parser
...
def default_config_parser(file_path, options):
    ...
    if hasattr(cfg, "test_only") and hasattr(cfg, "hooks"):
        for hook in cfg.hooks:
            if hook.get("type") == "PreciseEvaluator":
                hook["test_last"] = cfg.test_only
                break
...
def default_setup(cfg):
    ...
    assert cfg.batch_size % world_size == 0, "Batch size should be divided by gpu nums."
