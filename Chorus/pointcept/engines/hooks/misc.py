...
from pointcept.engines.test_lang import LangPretrainMultiTeacherTester
...
class InformationWriter(HookBase):
    def __init__(self):
        ...
        self._extra_output_items = []

    def _flatten_output(self, data, prefix=""):
        """Yield (key, value) pairs from nested dicts using '/' as separator."""
        for key, value in data.items():
            full_key = f"{prefix}{key}" if prefix == "" else f"{prefix}/{key}"
            if isinstance(value, dict):
                yield from self._flatten_output(value, full_key)
            else:
    ...
    def before_train(self):
        ...
        if self.trainer.cfg.enable_wandb:
            safe_wandb_define_metric("params/*", step_metric="Iter")
            safe_wandb_define_metric("train_batch/*", step_metric="Iter")
            safe_wandb_define_metric("train/*", step_metric="Epoch")
    ...
    def after_step(self):
        ...
        if "model_output_dict" in self.trainer.comm_info.keys():
            ...
            # multi-teacher pretainer return 'per_teacher_loss' dict, we flatten it here
            flattened_items = list(self._flatten_output(model_output_dict))
            numeric_keys = []
            for key, value in flattened_items:
                scalar_value = self._to_scalar(value)
                if scalar_value is not None:
                    self.trainer.storage.put_scalar(key, scalar_value)
                    numeric_keys.append(key)
                elif isinstance(value, str):
                    self._extra_output_items.append((key, value))
            self.model_output_keys = numeric_keys

        for key in self.model_output_keys:
            self.trainer.comm_info["iter_info"] += "{key}: {value:.4f} ".format(
                key=key, value=self.trainer.storage.history(key).val
            )
        for key, value in self._extra_output_items:
            self.trainer.comm_info["iter_info"] += f"{key}: {value} "
        ...
        if self.trainer.writer is not None:
            ...
            if self.trainer.cfg.enable_wandb:
                safe_wandb_log(
                    {"Iter": self.curr_iter, "params/lr": lr},
                    step=self.curr_iter,
...
class CheckpointSaver(HookBase):
    def __init__(self, save_freq=None, save_best_for=None):
    ...
    def after_epoch(self):
        ...
        if is_main_process() and self.trainer.cfg.evaluate:
            metrics_map = self.trainer.comm_info.get("current_metrics")
            if metrics_map:
                target_teachers = (
                    self.save_best_for if self.save_best_for is not None else metrics_map.keys()
                )
                for teacher_name in target_teachers:
                    metric_entry = metrics_map.get(teacher_name)
                    if metric_entry is None:
                        continue
                    metric_name = metric_entry["name"]
                    metric_value = float(metric_entry["value"])
                    best_entry = self.trainer.best_metrics.get(teacher_name)
                    if best_entry is None or metric_value > best_entry["value"]:
                        self.trainer.best_metrics[teacher_name] = {
                            "value": metric_value,
                            "name": metric_name,
                        }
                        updated_teachers[teacher_name] = metric_name
                        self.trainer.logger.info(
                            f"Best validation {metric_name} ({teacher_name}) updated to: {metric_value:.4f}"
                        )
                    else:
                        self.trainer.logger.info(
                            f"Currently Best {teacher_name}/{best_entry['name']}: {best_entry['value']:.4f}"
                        )

                if updated_teachers:
                    first_teacher = next(iter(updated_teachers))
                    self.trainer.best_metric_value = self.trainer.best_metrics[first_teacher][
                        "value"
                    ]

                if self.trainer.cfg.enable_wandb:
                    for teacher_name in self.trainer.best_metrics:
                        entry = self.trainer.best_metrics[teacher_name]
                        safe_wandb_log(
                            {
                                "Epoch": self.trainer.epoch + 1,
                                f"eval/best_{teacher_name}_{entry['name']}": entry["value"],
                            },
                            logger=self.trainer.logger,
                        )
            else:
                cur_val = self.trainer.comm_info.get("current_metric_value")
                cur_name = self.trainer.comm_info.get("current_metric_name")
                if cur_val is not None and cur_name is not None:
                    if cur_val > self.trainer.best_metric_value:
                        self.trainer.best_metric_value = cur_val
                        updated_teachers[None] = cur_name
                        self.trainer.logger.info(
                    ...
                    if self.trainer.cfg.enable_wandb:
                        safe_wandb_log(
                            {
                                "Epoch": self.trainer.epoch + 1,
                                "eval/best_" + cur_name: self.trainer.best_metric_value,
                            },
                            logger=self.trainer.logger,
                        )
        ...
        if is_main_process():
            ...
            torch.save(
                {
                    ...
                    "best_metrics": getattr(self.trainer, "best_metrics", {}),
                },
                f"{fname_last}.tmp",
            )
            ...
            if updated_teachers:
                first_key = next(iter(updated_teachers))
                shutil.copyfile(fname_last, ckpt_dir / "model_best.pth") # if any teacher is updated, save a copy to model_best.pth
                for teacher_name in updated_teachers:
                    if teacher_name is None:
                        continue
                    shutil.copyfile(fname_last, ckpt_dir / f"best_{teacher_name}.pth")
...
class CheckpointLoader(HookBase):
    ...
    def before_train(self):
        ...
        if self.trainer.cfg.weight and os.path.isfile(self.trainer.cfg.weight):
            ...
            num_model_keys = len(model_state.keys())
            weight = OrderedDict()
            ...
            # Detailed logging
            self.trainer.logger.info(
                f"Successfully loaded {len(weight)}/{num_model_keys} keys from the ckpt with {len(checkpoint['state_dict'])} keys."
            )
            if skipped_keys["shape_mismatch"]:
                self.trainer.logger.warning(
                    f"Skipped {len(skipped_keys['shape_mismatch'])}/{len(checkpoint['state_dict'])} keys due to shape mismatch:\n"
                )
            if skipped_keys["not_in_model"]:
                self.trainer.logger.warning(
                    f"Skipped {len(skipped_keys['not_in_model'])}/{len(checkpoint['state_dict'])} keys not in model:\n"
...
class PreciseEvaluator(HookBase):
    ...
    def after_train(self):
        ...
        if isinstance(cfg.test, dict):
            ...
            if self.test_last:
                ...
                base_ckpt_path = os.path.join(
                    self.trainer.cfg.save_path, "model", "model_best.pth"
                )
                self.trainer.logger.info("=> Testing on model_best...")
                load_checkpoint(tester.model, base_ckpt_path)
                self.trainer.logger.info(f"Loaded ckpt from {base_ckpt_path}")
            self._execute_precise_eval(tester, base_ckpt_path)
    ...
    def _execute_precise_eval(self, tester, base_ckpt_path: str = None) -> None:
        model_dir = os.path.join(self.trainer.cfg.save_path, "model")

        if isinstance(tester, LangPretrainMultiTeacherTester):
            teacher_sequence = tester._active_teachers()
            for teacher_name in teacher_sequence:
                teacher_ckpt = os.path.join(model_dir, f"best_{teacher_name}.pth")
                path_to_load = None
                if os.path.exists(teacher_ckpt):
                    path_to_load = teacher_ckpt
                    self.trainer.logger.info(
                        f"=> Testing teacher '{teacher_name}' with checkpoint: {teacher_ckpt}"
                    )
                elif base_ckpt_path and os.path.exists(base_ckpt_path):
                    path_to_load = base_ckpt_path
                    self.trainer.logger.info(
                        f"=> best_{teacher_name}.pth not found, falling back to {base_ckpt_path}"
                    )
                else:
                    self.trainer.logger.info(
                        f"=> best_{teacher_name}.pth not found, using current weights"
                    )

                if path_to_load:
                    load_checkpoint(tester.model, path_to_load)

                tester.test_teachers([teacher_name])
                torch.cuda.empty_cache()
