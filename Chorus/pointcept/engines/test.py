...
class ZeroShotSemSegTester(TesterBase):
    ...
    def test(self):
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            if reuse_pred:
                ...
                pred = torch.zeros((num_points, num_classes), device="cuda", dtype=torch.float16)
                ...
                for i, frag_dict in enumerate(fragment_list):
                    ...
                    # Forward pass
                    with torch.no_grad():
                        out_dict = self.model(input_dict, chunk_size=400000) # temp change from 600k to 400k
    ...
    def _log_final_metrics(self, final_record, save_path):
        ...
        if self.cfg.enable_wandb:
            safe_wandb_log(
                {
                    f"Eval/{self.cfg.data.test.type}/mIoU": mIoU,
                    f"Eval/{self.cfg.data.test.type}/mAcc": mAcc,
                    f"Eval/{self.cfg.data.test.type}/allAcc": allAcc,
                },
                logger=logger,
            )
        ...
        if self.excluded_indices:
            ...
            if self.cfg.enable_wandb:
                safe_wandb_log(
                    {
                        f"Eval/{self.cfg.data.test.type}/fg_mIoU": fg_mIoU,
                        f"Eval/{self.cfg.data.test.type}/fg_mAcc": fg_mAcc,
                        f"Eval/{self.cfg.data.test.type}/fg_allAcc": fg_allAcc,
                    },
                    logger=logger,
                )
        ...
        for i in range(self.num_classes):
            cls_name = f"Class_{i}-{self.class_names[i]}" if self.class_names else f"Class_{i}"
            logger.info(f"{cls_name} Result: iou/accuracy {iou_class[i]:.4f}/{accuracy_class[i]:.4f}")
            ...
            per_class_data.append([
                ...
                "excluded" if i in self.excluded_indices else "included",
                'presented' if union[i] > 0 else 'absent'
            ])
        if self.cfg.enable_wandb and per_class_data and is_wandb_active():
            per_class_table = wandb.Table(
                columns=["Class_ID", "Class_Name", "IoU", "Accuracy", "Status", "Presented"],
                data=per_class_data,
            )
            safe_wandb_log(
                {f"Eval/{self.cfg.data.test.type}/per_class_results": per_class_table},
                logger=logger,
            )
...
class SemSegTester(TesterBase):
    def __init__(
        self, cfg, model=None, test_loader=None, verbose=True, index=None, enable_voting=False,
        vote_k=25,
        ignore_index=-1, **kwargs
    ):
        super().__init__(cfg, model, test_loader, verbose, index=index, **kwargs)
        self.enable_voting = cfg.test.get("enable_voting", enable_voting)
        self.vote_k = cfg.test.get("vote_k", vote_k)
        self.ignore_index = cfg.test.get("ignore_index", ignore_index)
    ...
    def test(self):
        ...
        # Log voting configuration if enabled
        if self.enable_voting:
            logger.info(f"Neighbor voting enabled with k={self.vote_k}")
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            logger.info(f"scene_name: {data_name}")
            logger.info(f"origin_segment shape: {data_dict['origin_segment'].size}")

            # Check if prediction already exists (but need to re-evaluate if pc_coord is present)
            if os.path.isfile(pred_save_path) and "pc_coord" not in data_dict:
                logger.info(
                ...
                tensor_type = torch.float16 if data_dict['coord'].dtype == np.float16 else torch.float32
                pred = torch.zeros((segment.size, self.cfg.data.num_classes), dtype=tensor_type).cuda()
                pred_coord = torch.zeros((segment.size, 3), dtype=tensor_type).cuda()
                ...
                if "origin_segment" in data_dict.keys():
                    ...
                    logger.info(f"pred shape after inverse operation: {pred.shape}")
                    segment = data_dict["origin_segment"]
                    logger.info(f"segment shape after inverse operation: {segment.shape}")

                # Check if we use pc_segment for evaluation
                if "pc_coord" in data_dict and "pc_segment" in data_dict:
                    segment = data_dict["pc_segment"]
                    logger.info(f"using pc_segment for evaluation with shape {segment.shape}")
                    if not self.enable_voting:
                        self.enable_voting = True  # Required enable_voting if eval on pc_segment

                # neighbor voting on pred if enabled
                # currently enable_voting will lose the top-3 pred for ScanNetPP
                if self.enable_voting:
                    num_classes = self.cfg.data.num_classes
                    ignore_index = self.ignore_index
                    if data_dict.get("origin_feat_mask", None) is not None:
                        logger.info((f"using valid_mask {sum(data_dict['origin_feat_mask'])}/{len(data_dict['origin_feat_mask'])} points for voting"))

                    if "pc_coord" in data_dict and "pc_segment" in data_dict:
                        # Use origin_coord as base coordinates and pc_coord as query
                        coords = data_dict.get("origin_coord", pred_coord)
                        query_coords = data_dict["pc_coord"]
                        logger.info(f"applying neighbor voting with pc_coord as query (k={self.vote_k})")
                        
                        pred = neighbor_voting(
                            coords,
                            pred[:, 0] if pred.ndim > 1 else pred,  # use top-1 if top-3
                            self.vote_k,
                            ignore_index,
                            num_classes,
                            valid_mask=data_dict.get("origin_feat_mask", None),
                        ...
                        # Standard voting with origin coordinates
                        coords = data_dict["origin_coord"]
                        logger.info(f"applying standard neighbor voting (k={self.vote_k})")
                        
                        pred = neighbor_voting(
                            coords,
                            pred[:, 0] if pred.ndim > 1 else pred,
                            self.vote_k,
                            ignore_index,
                            num_classes,
                            valid_mask=data_dict.get("origin_feat_mask", None),
                        ...
                        logger.warning(
                            "Neighbor voting requires 'origin_coord' or 'pc_coord' in data_dict, skipped.."
                        )
                    
                    # Apply clustering voting if instance information is available
                    if "origin_instance" in data_dict:
                        logger.info("Applying clustering voting")
                        pred = clustering_voting(
                            pred, data_dict["origin_instance"], ignore_index
                        )
            ...
            if (
                ...
                pred = pred[:, 0] if pred.ndim > 1 else pred  # for mIoU, handle both 1D and 2D pred
            elif self.cfg.data.test.type == "SemanticKITTIDataset":
                # 00_000000 -> 00, 000000
                sequence_name, frame_name = data_name.split("_")
                os.makedirs(
            ...
            # Add indicator if using pc_segment for evaluation
            eval_type = ""
            if "pc_coord" in data_dict and "pc_segment" in data_dict:
                eval_type = "[PC Label Eval. ] "
        ...
        if comm.is_main_process():
            ...
            if self.cfg.enable_wandb:
                ...
                if per_class_data and is_wandb_active():
                    per_class_table = wandb.Table(
                        columns=["Class_ID", "Class_Name", "IoU", "Accuracy", "Presented"],
                        data=per_class_data,
                    )
                    safe_wandb_log(
                        {f"Eval/{self.cfg.data.test.type}/per_class_results": per_class_table},
...
from .test_lang import *  # noqa: F401,F403
