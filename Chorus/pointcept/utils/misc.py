...
def cancel_and_terminate_pool(pool, futures=None):
    """Cancel pending futures and terminate worker processes."""
    import sys
...
def inspect_memory(top_n=25):
    """
    Inspects memory to find the largest torch.Tensors and np.ndarrays.
    This version is robust against ReferenceErrors from dead weak references.
    """
    ...
    for obj in gc.get_objects():
        try:
            if isinstance(obj, torch.Tensor):
                ...
                large_objects.append((size, type(obj), obj.shape, obj.device))
            elif isinstance(obj, np.ndarray):
                size = obj.nbytes
                large_objects.append((size, type(obj), obj.shape, 'numpy'))
        except ReferenceError:
            # This object is a dead weak reference, just ignore it
            continue

    # Sort by size in descending order
    large_objects.sort(key=lambda x: x[0], reverse=True)
    
    # Print the top N largest objects
    print(f"\n--- Top {top_n} Largest NumPy/Torch Objects in Memory ---")
    for size_bytes, obj_type, shape, device in large_objects[:top_n]:
        size_mb = size_bytes / (1024**2)
        print(f"- {size_mb:8.2f} MB | Type: {str(obj_type):<30} | Shape: {str(shape):<25} | Device: {device}")
    print("--------------------------------------------------------\n")
