...
seed = 1219  # train process will init a random seed and record
...
nan_detection = False # PyTorch forward hooks, will slow down training
set_detect_anomaly = False # torch.autograd.set_detect_anomaly(True)

enable_wandb = True
wandb_project = "scene_3dgs_pro"  # custom your project name e.g. Sonata, PTv3
wandb_key = None  # wandb token, default is None. If None, login with `wandb login` in your terminal
wandb_id = None  # wandb run id, resume wandb run when not None
