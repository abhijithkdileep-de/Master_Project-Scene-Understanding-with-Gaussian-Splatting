"""Apply various augmentations to a sample ply file and save the results for visualization and analysis."""
...
from pointcept.datasets.transform import (  # noqa
    RandomDropout,
    RandomRotate,
    RandomFlip,
...
def main():
    input_file = "/path/to/example/plyfile"
    data_dict = load_ply(input_file)
    print("Loaded input file.")

    # For testing, we force the augmentation to apply.
    transforms = {
        ...
        # Rendering Equivalence Augmentations, set p=1.0 to always apply here
        "GaussianNoiseInjection": GaussianNoiseInjection(noise_lr=1e-4, p=1.0),
        "GaussianRelocation": GaussianRelocation(
            min_opacity=0.006,
            relocation_rate=0.20,
            p=1.0,
        ),

        # Immature Manifold Augmentations
        "GaussianScaleBlur": GaussianScaleBlur(blur_factor_range=[3.0, 4.0], p=1.0),
        "GaussianFrequencyNoise": GaussianFrequencyNoise(
            high_freq_noise=0.005,
            p=1.0,
        ),
    }
