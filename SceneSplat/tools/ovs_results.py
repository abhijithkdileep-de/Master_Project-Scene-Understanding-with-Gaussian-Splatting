"""SceneSplat-style comparison artifacts for SceneSplat 3D OVS evaluation.
...
def save_query_artifacts(output_dir, query, all_queries, scores, selection, summary, results, scene, args):
    ...
    write_json(artifact('retrieval',f'{slug}_retrieval.json'), dict(
        method='SceneSplat', scene=args.scene, query=query, selection_method='scene_class_cosine_argmax',
...
def save_comparison_reports(output_dir, report):
    ...
    metrics=dict(method='SceneSplat',scene=report['scene'],protocol=report['protocol'],neighbor_voting=False,
    ...
    with (output_dir/'per_query_results.csv').open('w',newline='') as stream:
        ...
        for row in rows:
            writer.writerow(['SceneSplat',report['scene'],row['query'],round(row['mean_iou'],6),round(row['mAcc'],6),
                row['selected_gaussians']])
    manifest=dict(method='SceneSplat',scene=report['scene'],protocol=report['protocol'],neighbor_voting=False,
