"""Evaluate input-aligned SceneSplat language features on 3D OVS PNG masks."""
import argparse
import contextlib
import csv
import io
...
def parse_args():
    ...
    p.add_argument('--feature-path', type=Path, help='Saved SceneSplat language feature tensor')
    ...
    p.add_argument('--text-model', default='google/siglip2-base-patch16-512')
...
def load_scene(root, feature_path):
    ...
    if np.any(scene['scale'] <= 0) or np.any((scene['opacity'] < 0) | (scene['opacity'] > 1)):
        raise ValueError('Expected physical positive scales and opacity probabilities, as exported by SceneSplat preprocessing')
...
def encode_text(labels, args):
    ...
    if args.text_embeddings:
        ...
        encoder = SigLIPLabelEncoder(model_name=args.text_model.removeprefix('google/'), device=args.device)
        # Match SceneSplat LERF's default 'this is a {label}' prompts.
        features = encoder.encode_labels(labels)
...
def main():
    ...
    qualitative_manifest = save_semantic_visualizations(
        output_dir, frames, results_by_query, all_queries, args, 'SceneSplat')
    rows, summaries = [], []
    ...
    report = dict(scene=args.scene, dataset='3dovs', protocol='cosine_argmax_segmentation',
                  ...
                  geometry_coordinate_space='COLMAP_world', text_prompt='this is a {label}',
